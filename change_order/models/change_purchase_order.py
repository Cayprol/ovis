...
class ChangePurchaseOrder(models.Model):

	_name = 'change.purchase.order'
	_description = 'Change Purchase Order'
	...
	def create(self, vals):
		vals['name'] = self.env['ir.sequence'].next_by_code('change.purchase.order') or _('New')

		record = super(ChangePurchaseOrder, self).create(vals)
	...
	def write(self, vals):
		...
		return super(ChangePurchaseOrder, self).write(vals)

	name = fields.Char(string='Change Order', copy=False, readonly=True, index=True, default=lambda self: _('New'))
	purchase_order_id = fields.Many2one('purchase.order', string='Purchase Order', domain="[('state','=','purchase')]", required=True)
	purchase_order_line = fields.One2many(related='purchase_order_id.order_line', readonly=True, string="Purchase Order Lines")
	...
	approved_purchase_order_line = fields.One2many('purchase.order.line', 'approved_order_id', states=READONLY_STATES, string='Approved Purchase Order Lines') # copy=True, auto_join=True
	original_purchase_order_line = fields.One2many('purchase.order.line', 'original_order_id', readonly=True, string='Original Purchase Order Lines') # copy=True, auto_join=True

	partner_id = fields.Many2one(related='purchase_order_id.partner_id', readonly=True)
	company_id = fields.Many2one(related='purchase_order_id.company_id', readonly=True)
	...
	currency_id = fields.Many2one("res.currency", related='purchase_order_id.currency_id', string="Currency", readonly=True, required=True)
		...
		for order in self:
			if self.env.user.has_group('purchase.group_purchase_manager'):
				...
				if self.env.user.partner_id:
					order.purchase_order_id.message_subscribe(partner_ids=[self.env.user.partner_id.id])
	...
	def action_done(self):
		for order in self:
			for source_line in order.purchase_order_line:
				source_line.update({
					'original_order_id': order.id,
					'order_id': False,
					})

			for approved_line in order.approved_purchase_order_line:
			...
			order.purchase_order_id.message_post(body=thread_msg)
			if self.env.user.partner_id:
				order.purchase_order_id.message_subscribe(partner_ids=[self.env.user.partner_id.id])
	...
	def _track_subtype(self, init_values):
		...
		return super(ChangePurchaseOrder, self)._track_subtype(init_values)

	@api.depends('approved_purchase_order_line.price_total')
	def _changed_amount_all(self):
		...
		for order in self:
			...
			for line in order.approved_purchase_order_line:
	...
	@api.depends('original_purchase_order_line.price_total')
	def _original_amount_all(self):
		...
		for order in self:
			amount_untaxed = amount_tax = 0.0
			for line in order.original_purchase_order_line:
				amount_untaxed += line.price_subtotal
				amount_tax += line.price_tax
			order.update({
